...
class Input(Node):
    ...
    def __init__(self, input=None, source = None, output=None, input_file=None, name=None):
        """
...
class Graph(object):
    ...
    def __init__(self, source, input_node, output_node, name=None):
        """
    ...
    def run(self, input_file=None,
            output_file=None, verbose=False, **kwargs):
        """
        ...
        if verbose:
            print("Computing in {}\n".format(self.name))
        res = []
        data_flow = kwargs[self.source]
        if data_flow is not None:
            for graph, file in data_flow.items():
